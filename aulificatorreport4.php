		...
		<?php
				...
				foreach ($aulas as $cod => $detalle) {
					for ($i = 0; $i < 12; $i++) {
						foreach ($dias as $dia) {
							...
							$datosReporte[$letra][$cod][11]['Aula'] = $detalle['capacidad'] . 'A';
			...
			while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
				...
				if ($row['tipoHorario'] == 'completo') {
					...
					$datosReporte[$row['turno']][$row['aula']]['partido'][$row['dia']] = true;
					$datosReporte[$row['turno']][$row['aula']][7][$row['dia']] = $row['nombre'];
					$datosReporte[$row['turno']][$row['aula']][8][$row['dia']] = $horasTurno[$row['horario']];
					$datosReporte[$row['turno']][$row['aula']][9][$row['dia']] = $row['cantidad_alumnos'] . " Alumnos";
					$datosReporte[$row['turno']][$row['aula']][10][$row['dia']] = $row['materia'] . $row['comision'];
				}
			}
			...
			foreach($turnos as $letra => $turno) {
				...
				foreach ($aulas as $aula => $detalle) {
					for ($i = 0; $i <12; $i++) {
							...
							foreach ($dias as $dia) {
								$partido = "";
								if ($datosReporte[$letra][$aula]['partido'][$dia] and $i == 6) {
