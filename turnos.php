	...
	<script>
		$(document).ready(function() {
			...
			var actualizarTabla = function() {
				...
				$('#tablaDatos').load("fuentes/AJAX.php?act=tablaTurnos", formValues, function(data) {
					//$('.botonEliminar').off();
			...
			$('#formularioCarga').submit( function(event) {
				...
				$.post("./fuentes/AJAX.php?act=agregarTurno", values, function(data) {
					//console.log('data');
					...
					if (data.hayErrores == 1) {	
						var error = "El turno agregado se superpone con las siguientes materias: \n";
						$.each(data.errores, function(key, value) {
						
							error += value.cod + " " + value.nombre + " Plan: " + value.plan + "(" + value.dia  + " " + value.turno + ")\n";
						});
						//var error = "test";
						alert(error);
					} else {				
