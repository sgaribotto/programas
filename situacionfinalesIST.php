...
<html>
	<head>
		...
		<title>Situación Finales IST</title>
		<?php 
	...
	<body>
		...
		<div class="formularioLateral">
			<h2 class="formularioLateral">Situación Finales IST</h2>
			...
			<div id="filtros" class="desplegable">
				<fieldset class="formularioLateral">
					<form method="post" class="formularioLateral filtros" action="" >
						...
						<select class="formularioLateral iconCod" name="carrera" required="required" id="carrera" >
							...
							<?php
								...
								$query = "SELECT DISTINCT carrera, nombre_carrera
											...
											WHERE nombre_ua = 'EEyN-04 Escuela de Economía y '
												AND tipo = 'Final'
											ORDER BY nombre_carrera";
								$result = $mysqli->query($query);
								
								while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
									echo "<option value='{$row['carrera']}'>({$row['carrera']}) {$row['carrera_nombre']}</option>";
								}	
						...
						<select class="formularioLateral iconCod" name="materia" required="required" id="materia" >
							<?php
								require 'fuentes/conexion.php';
								...
								while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
									echo "<option value='{$row['materia']}'>({$row['materia']}) {$row['nombre_materia']}</option>";
								}	
	...
	<script>
		$(document).ready(function() {
			...
			var actualizarTabla = function() {
				...
				$('#tablaDatos').load("fuentes/AJAX.php?act=tablaSituacionFinalesCONEAUIST", formValues, function(data) {
